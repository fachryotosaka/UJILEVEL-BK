...
<div id="detailrequest" tabindex="-1" aria-hidden="true" class="modal hidden z-10 box-border fixed inset-0 p-20 px-32 py-24 pb-10 flex items-center justify-center overflow-hidden overscroll-contain bg-header/30 transition duration-200 pointer-events-auto visible opacity-100 [&>*]:translate-y-0 [&>*]:scale-100">
    ...
    <div class="flex items-center gap-4 mt-10 font-Mplus1cus">
        ...
        @if(Auth::user()->role === 'teacher')
            <a id="result-form" class="px-5 py-3 rounded-md text-[#EAEBEF] ml-auto font-bold text-xs bg-[#15A77D] hover:scale-95 transition duration-200">Result</a>
        @endif
    </div>
</div>
