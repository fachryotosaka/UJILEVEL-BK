@switch(true)
    ...
    <div class="flex items-center w-full">
        ...
        <label for="search" class="flex items-center gap-3 py-3 w-96 px-3  rounded-md border border-subheader border-opacity-0 hover:border-opacity-20 focus-within:border-opacity-20 shadow transition-all duration-300 ease-in-out group/form">
            ...
            <input id="search" type="text" class=" m-0 text-xs font-medium text-subheader bg-transparent placeholder:opacity-8 tracking-wide pr-1 focus:outline-none w-full " placeholder="Search...">
...
<div class="flex items-center min-w-fit ml-6">
    ...
    <label for="tw-modal" data-id="inbox" class="cmodal cursor-pointer w-10 h-10 justify-center border border-subheader border-opacity-50 rounded-full flex items-center hover:border-opacity-90 group/hover transition-all ease-in-out duration-300 hover:scale-95 m-0">
    ...
    <div class="w-11 h-11 rounded-full bg-cover m-0" style="background-image: url({{ Auth()->user()->profile_photo_url }})"></div>

    <div class="flex flex-col font-Mplus1cus ml-2 -mt-0.5 ">

        <p class="font-semibold text-black-main m-0">{{Auth::user()->name}}</p>
        <p class="font-medium text-[11px] text-footer opacity-50 m-0">{{Auth::user()->role}}</p>
